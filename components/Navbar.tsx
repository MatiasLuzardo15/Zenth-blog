        ...
        <div className="flex justify-between items-center">
          ...
          >
            <div className="relative">
                ...
                <PenTool className="h-6 w-6 relative z-10 text-black dark:text-white transform -rotate-12" strokeWidth={2.5} />
            </div>
            <span className="ml-2 text-3xl font-serif font-black text-black dark:text-white tracking-tighter decoration-wavy underline decoration-zenth-400">zenth</span>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-4">
            {navLinks.map((item) => (
               <button 
                 ...
                 className={`px-4 py-1 text-lg font-bold transition-colors relative group ${
               ...
               >
                 {item.name}
                 <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-black dark:bg-white transform transition-transform origin-left ${
                    currentPage === 'blog' && item.name === 'Blog' ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'
                 }`}></span>
               </button>
            ))}
